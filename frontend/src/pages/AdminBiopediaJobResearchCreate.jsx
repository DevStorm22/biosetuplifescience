...
export default function AdminBiopediaJobResearchCreate() {
  const [message, setMessage] = useState("");

  useEffect(() => {
    axios
      .get("http://localhost:5000/biopediajobsresearch/")
      .then((res) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:5000/biopediajobsresearch/create",
        payload
      );
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen">
      <h1 className="text-4xl font-bold mb-4">
        Admin Biopedia Job Research Create Page
      </h1>
